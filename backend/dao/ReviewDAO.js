...
module.exports = class ReviewDAO {
  ...
  static async createReview (body) {
    ...
    await db
      ...
      .then((assignment) => {
        if (assignment.rows.length <= 0) { throw new Error('This help counter is not open') }
      })
    ...
    await db
      ...
      .then((student) => {
        if (student.rows.length <= 0) { throw new Error('This student is not allowed to request help') } else studentId = student.rows[0].id
      })
    ...
    return db
      ...
      .then(async (students) => {
        for (let i = 0; i < students.rows.length; i++) {
          if (students.rows[i].reviewer_id == null) { throw new Error('This student has already requested help') }
        }
