...
exports.login = (req, res, next) => {
  ...
  AuthDAO.getFullAccountByUsername(escape(username).trim())
    .then(user => {
      ...
      if (!loadedUser) { return res.status(401).json({ message: 'Invalid login.' }) }
...
exports.register = async (req, res, next) => {
  ...
  if (!REGISTER_KEY || !body.key || body.key != REGISTER_KEY) { return res.status(422).json({ message: 'Not allowed to register' }) }

  if (body.username) body.username = escape(body.username).trim()

  if (!matches(body.username, '^[a-zA-Z0-9_\.\-]*$')) { return res.status(422).json({ message: 'Invalid characters in username' }) } else if (body.username.length < 5) { return res.status(422).json({ message: 'Username too short' }) }
